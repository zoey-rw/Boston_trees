#### prepare Boston map data #####
  library(googlesheets)
  library(tigris)   # census data for map
  
  #### prepare community organization data ####
...
neighborhood <- read_csv("Climate_Ready_Boston_Social_Vulnerability.csv")
...
community$community_org_link <- paste("Community organizations:<br>",
...
#### read in data
asthma <- read_csv("Chronic Asthma Prevalence.csv")    # asthma prevalence
hvi <- readxl::read_xlsx("Boston_HVI_original_Mean_LST_joined.xlsx") # HVI 
#hvi <- readxl::read_xlsx("Boston_HVI_2011_LST_6_categories.xlsx") # HVI 
current_percent <- readxl::read_xls("Boston_CTs_Percent_Current_Veg.xls", sheet = "Sheet1") # current vegetation per tract, as percentage
potential_percent <- readxl::read_xls("Boston_CTs_Percent_Potential_Veg.xls", sheet = "Sheet1") # potential vegetation per tract, as percentage
current.potential_total <- readxl::read_xlsx("Boston_CTs_Total_CurrentPotential_Veg.xlsx") # current/potential vegetation per tract, sq ft


tracts <- tracts("massachusetts", county = "Suffolk", year=2010) # download tract data (using Tigris package)
...
layers <- Reduce(function(...) merge(..., all=TRUE), list(hvi, asthma, current_percent, potential_percent, current.potential_total, community)) ## combine all layers
...
saveRDS(tract_data, "tract_data.rds")
